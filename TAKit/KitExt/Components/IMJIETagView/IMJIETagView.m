...
-(void)setClickString:(NSString *)clickString{
    ...
    if ([_tagsFrame.tagsArray containsObject:_clickString]) {
        ...
        NSInteger index = [_tagsFrame.tagsArray indexOfObject:_clickString]+1;
    ...
    for (id obj in self.subviews) {
        if ([obj isKindOfClass:[UIButton class]]) {
            ...
            if (btn.tag == index){
                ...
                btn.backgroundColor = _clickBackgroundColor;//[UIColor whiteColor];TODO
                //btn.alpha = 0.5;
...
//                [self makeCorner:_clickborderSize view:btn color:_clickTitleColor];
                [self makeCorner:_clickborderSize view:btn color:UIColorRGBA(248, 96, 60, 1)];
